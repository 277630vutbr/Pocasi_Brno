...
def build_projections(annual: pd.DataFrame, scenarios: dict):
    base_year = 2025
    baseline_temp = annual['tavg'].loc[:str(base_year)].tail(30).mean()
    baseline_prcp = annual['prcp'].loc[:str(base_year)].tail(30).mean()
    baseline_wind = annual['wind'].loc[:str(base_year)].tail(30).mean()
    targets = [2035, 2125, 3025]
    rows = []
    ...
    return pd.DataFrame(rows).sort_values(['scenario','target_year'])
...
start = st.sidebar.date_input("Začátek období", value=date(2000,1,1))
end = st.sidebar.date_input("Konec období", value=date(2025,12,31))
station = st.sidebar.text_input("Meteostat Station ID", value=DEFAULT_STATION_ID)
run = st.sidebar.button("Načíst a zpracovat")

# Enforce fixed window [2000-01-01, 2025-12-31]
start = max(start, date(2000,1,1))
end = min(end, date(2025,12,31))
