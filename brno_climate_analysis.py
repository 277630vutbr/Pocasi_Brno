...
START = datetime(2000, 1, 1)     # adjustable
END_CEILING = datetime(2025, 12, 31)
END   = min(datetime.today(), END_CEILING)
...
def build_projections(annual, scenarios):
    # Projections for fixed target years with baseline 2025
    base_year = 2025
...
def to_pdf(annual, projections, out_pdf):
    with PdfPages(out_pdf) as pdf:
        ...
        fig.text(0.5, 0.64, f"Zpracování: Python (pandas, matplotlib) – {datetime(2025, 12, 31).date()}", ha='center', fontsize=10)
